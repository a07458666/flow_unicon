...
class webvision_dataset(Dataset): 
    def __init__(self, root_dir, transform, sample_ratio, mode, num_class, pred=[], probability=[]): 
        self.root = root_dir
        self.transform = transform
        self.mode = mode  
...
class webvision_dataloader():  
    ...
    def run(self,SR, mode, pred=[],prob=[]):
        if mode=='warmup':
            ...
            labeled_dataset = webvision_dataset(root_dir=self.root_dir, transform = self.transforms["labeled"],  sample_ratio = SR, mode="labeled",num_class=self.num_class,pred=pred,probability=prob)              
            ...
            unlabeled_dataset = webvision_dataset(root_dir=self.root_dir, transform = self.transforms["unlabeled"],  sample_ratio = SR, mode="unlabeled",num_class=self.num_class,pred=pred)                    
            ...
            val_dataset = webvision_dataset(root_dir=self.root_dir, transform = self.transforms["test"],  sample_ratio = SR, mode='val', num_class=self.num_class)      
